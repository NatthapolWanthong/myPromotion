...
export class modalPromotion {
  constructor(type, target, campaign_id) {
    this.type = type;
    this.target = target;
  ...
  putOption() {
  const typeSelect = FormHelper.generateOptions(this.type);
  const targetSelect = FormHelper.generateOptions(this.target);
  ...
  bindEvents() {
    ...
    toggle.addEventListener("change", () => {
      if (toggle.checked) {
        ...
        codeInput.value = this.generateAutoCode();
      } else {
    ...
    API.insertPromotion(formData)
      .then((res) => {
        if (res.success) {
          ...
          if(isPromotionPage()){
            goToPromotionPage(this.campaign_id)
          }else {
