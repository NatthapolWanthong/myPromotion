...
if ($id) {
  // ========== ดึงเฉพาะ Promotion ตาม ID ========== 
  $sql = "SELECT id, name, type, target, start_date, end_date, status, created_by, create_date, edit_date, promotion, code, location, note, description, campaign_id
        FROM promotion
        WHERE id = ?";
...
// Query Count ทั้งหมด
$countSql = "SELECT COUNT(*) FROM promotion $whereSQL";
...
$statusCountSql = "SELECT status, COUNT(*) as cnt FROM promotion $whereSQL GROUP BY status";
...
// Query Data
$dataSql = "SELECT id, name, type, target, start_date, end_date, status, code, created_by, create_date, edit_date, promotion, location, note, description, campaign_id
            FROM promotion $whereSQL
